...
def _output_path_for(cfg: Dict[str, Any], file_path: Path, *, vs_idx: int, mode: str) -> Path:
    """
    Resolve the expected output file path for a given run configuration.
    
    Dynamically branches based on the `auto_cc` config flag to match the 
    outputs generated by the correlation engine:
    - Auto-Correlation: `[date]_auto_[mode].npy`
    - Cross-Correlation: `[date]_cc_[vs]_[mode].npy`
    
    :param cfg: The overridden configuration dictionary for the current run.
    :param file_path: The original input .npz file path.
    :param vs_idx: The virtual source index (ignored if auto_cc=True).
    :param mode: The correlation mode ("conventional" or "v1").
    :return: The fully resolved Path object for the expected output.
    """
    ...
    if is_auto:
        return out_root / basename.replace(".npz", f"_auto_{mode}.npy")
    else:
...
class BenchmarkRunner:
    ...
    def _run_pool(self, cfg: Dict[str, Any]) -> float:
        """
        Run CC on self.files with multiprocessing.Pool.
        Returns makespan (wall sec).
        """
        njobs = int(get_cfg(cfg, ["runtime", "njobs"], 1))
        njobs = max(1, njobs)
        ...
        # FIX: Use Pool with maxtasksperchild=1 to physically flush RAM after every file
        with mp.Pool(processes=njobs, initializer=initializer, maxtasksperchild=1) as pool:
            # Pack arguments for the unpacker
            task_args = [(str(f), cfg) for f in self.files]
            
            # OPTIMIZATION: Use imap_unordered for memory efficiency and instant exception raising
            for _ in pool.imap_unordered(_process_unpack, task_args, chunksize=1):
                pass
    ...
    if not dcp_all.empty:
        ...
        for gwin in _group_by_window(dcp_all):
            ...
            if "cos_p05" in v1f.columns and v1f["cos_p05"].notna().any():
                ...
                ax.set_title(f"Fidelity Error: (1.0 - cos_p05) vs max lag {win_label}".strip(), pad=20)
                ax.set_xlabel("max_lag_sec (s)")
                ax.set_ylabel("Error (1.0 - cos_p05)")

                # Log Scale & Scientific Notation Formatter 
                ax.set_yscale("log")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Robustness & performance benchmark for Cross-Correlation and Auto-Correlation engines.")
    ...
    # Auto-CC override
    p.add_argument("--auto_cc", action="store_true", help="Force Auto-Correlation mode for the benchmark.")
...
def main() -> None:
    ...
    # Inject CLI override into the base config
    if args.auto_cc:
        if "xcorr" not in cc_cfg:
            cc_cfg["xcorr"] = {}
    ...
    if args.cores is not None and len(args.cores) > 0:
        ...
        # OPTIMIZATION: Ensure the scaling sweep doesn't test more cores than SLURM allocated
        slurm_cores = os.environ.get("SLURM_CPUS_PER_TASK")
