...
:purpose: Multi-Channel Cross-Correlation (MCCC) for DAS array processing.
          ...
          following VanDecar & Crosson (1990).

Notes:
    - ``bandpass_filter_tukey`` lives in ``src.ani`` (the preprocessing
      module), not ``src.utils``.
    - ``tukey`` is imported from ``scipy.signal.windows`` (the top-level
      ``scipy.signal.tukey`` alias was removed in SciPy >= 1.13).
"""
...
def torch_xcorr_1d_vs_nd(signal_1: torch.Tensor, signal_2: torch.Tensor) -> torch.Tensor:
    ...
    :return: Cross-correlation result, perfectly centered (length 2*npts - 1;
             zero lag at index npts - 1).
    :rtype: torch.Tensor
    """
...
def compute_mccc_delays(
    ...
    # Z-score each channel so the pairwise correlations are amplitude-free
    # (after the 1/npts scaling below, a perfectly aligned identical pair
    # scores ~1).
    ...
    logger.debug("Starting pairwise cross-correlation for %d channels...", nch)
    ...
    logger.debug("Solving linear system with %d equations for %d channels...", nrow, ncol)
...
def run_mccc_windowed(
    ...
    if pick_time <= 0:
        raise ValueError(f"pick_time must be > 0 (got {pick_time}).")
    ...
    upsample_factor = max(1, int(np.ceil(5 / (t2 - t1))))
    idx_t1 = int(np.floor(t1 / dt * upsample_factor))
    idx_t2 = int(np.ceil(t2 / dt * upsample_factor))

    logger.info("Running MCCC. Upsampling factor: %dx. Target window: [%.2fs, %.2fs]",
                upsample_factor, t1, t2)
    ...
    # Clamp the crop window to the available samples (the requested window can
    # exceed the folded trace length when pick_time + half_width > max lag).
    idx_t2 = min(idx_t2, data_up.shape[-1])
    if idx_t2 - idx_t1 < 2:
        raise ValueError(
            f"Crop window [{t1:.2f}s, {t2:.2f}s] falls outside the folded NCF "
            f"({data_up.shape[-1]} samples at dt={dt / upsample_factor:.6f}s)."
        )
    ...
    # Move to PyTorch
    data_tensor = torch.from_numpy(np.ascontiguousarray(data_cropped)).to(device)
    ...
    # NOTE: compute the mask ONCE before writing NaNs. The previous
    # implementation NaN'd ccmax first and then re-evaluated
    # `ccmax < cc_threshold` for dtmax; NaN comparisons are always False, so
    # dtmax kept exactly the low-quality entries that should have been
    # discarded.
    low_quality = ccmax < cc_threshold
    ccmax[low_quality] = np.nan
    dtmax[low_quality] = np.nan
