...
module load py-triton/3.1.0_py312
module load py-cupy/13.6.0_py312
...
try:
    ...
    print(f"⚠️ WARNING: Missing GPU array dependency: {e}")
    print("    Make sure the py-cupy module is loaded (e.g., module load py-cupy/13.6.0_py312).")

# Verify Triton (Required for torch.compile)
try:
    import triton
    print("Triton:", triton.__version__)
except ImportError as e:
    print(f"⚠️ WARNING: Missing compiler dependency: {e}")
    print("    Triton is required if torch_compile=True. Make sure the py-triton module is loaded.")
